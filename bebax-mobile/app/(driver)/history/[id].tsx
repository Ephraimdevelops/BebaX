...
export default function DriverHistoryDetail() {
    ...
    // Fetch real ride data - skip query if id is not available
    const ride = useQuery(
        api.rides.getRide,
        id ? { ride_id: id as Id<"rides"> } : "skip"
    );
    ...
    if (ride === undefined) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={{ marginTop: 16, color: Colors.textDim }}>Loading trip details...</Text>
            </View>
        );
    }
    ...
    const handleCallCustomer = () => {
        if (ride.customer_details?.phone) {
            Linking.openURL(`tel:${ride.customer_details.phone}`);
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={styles.content} contentContainerStyle={{ paddingBottom: 40 }}>
                ...
                {/* Customer Info Card */}
                {ride.customer_details && (
                    <View style={styles.driverCard}>
                        <Image
                            source={{ uri: ride.customer_details.photo || 'https://i.pravatar.cc/150' }}
                            style={styles.driverImage}
                        />
                        <View style={styles.driverInfo}>
                            <Text style={styles.driverName}>{ride.customer_details.name || 'Unknown User'}</Text>
                            <Text style={styles.vehicleInfo}>Customer</Text>
                        </View>
                ...
                <View style={styles.sectionCard}>
                    ...
                    <View style={styles.rowBetween}>
                        ...
                        <Text style={styles.rowValue}>TSh {(ride.final_fare || 0).toLocaleString()}</Text>
                    </View>
                    <View style={[styles.rowBetween, { marginTop: 8 }]}>
                        <Text style={styles.rowLabel}>Commission (10%)</Text>
                        <Text style={[styles.rowValue, { color: Colors.danger }]}>
                            - TSh {((ride.final_fare || 0) * 0.1).toLocaleString()}
                        </Text>
                    </View>
                    <View style={styles.divider} />
                    <View style={styles.rowBetween}>
                        <Text style={[styles.rowLabel, { fontWeight: '700', color: Colors.text }]}>Net Earnings</Text>
                        <Text style={[styles.rowValue, { color: Colors.success, fontSize: 16 }]}>
                            TSh {((ride.final_fare || 0) * 0.9).toLocaleString()}
                        </Text>
                    </View>
                </View>
                ...
                <View style={styles.actionContainer}>
                    <TouchableOpacity style={styles.mainButton} onPress={handleCallCustomer}>
                        <Ionicons name="call" size={20} color="white" style={{ marginRight: 8 }} />
                        <Text style={styles.mainButtonText}>Call Customer</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={styles.secondaryAction}>
                        <MaterialIcons name="flag" size={16} color={Colors.textDim} style={{ marginRight: 6 }} />
                        <Text style={styles.secondaryActionText}>Report an issue</Text>
                    </TouchableOpacity>
                </View>
            </ScrollView>
        </View>
...
const styles = StyleSheet.create({
    ...
    subText: { fontSize: 10, color: Colors.textDim },
