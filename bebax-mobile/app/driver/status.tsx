...
type VerificationStatus = 'pending' | 'verified' | 'action_required' | 'rejected';

export default function DriverStatus() {
    const insets = useSafeAreaInsets();
    const router = useRouter();

    // Check driver status from backend
    const driverProfile = useQuery(api.drivers.getMyDriver);

    // Derive status
    const status: VerificationStatus = driverProfile?.verified
        ? 'verified'
        : driverProfile?.verification_status === 'action_required'
            ? 'action_required'
            : driverProfile?.verification_status === 'rejected'
                ? 'rejected'
                : 'pending';

    const rejectionReason = driverProfile?.rejection_reason || null;
    const rejectionMessage = driverProfile?.rejection_message || null;

    // If approved, redirect to cockpit
    useEffect(() => {
        if (driverProfile?.verified === true) {
    ...
    const statusConfig = {
        ...
        verified: {
            ...
            iconBg: '#D1FAE5',
            title: 'Approved!',
            subtitle: 'Congratulations! You can now start accepting rides.',
        },
        action_required: {
        ...
        rejected: {
            ...
            subtitle: 'Unfortunately, your application could not be approved. Contact support for details.',
        },
    };
                ...
                {status === 'pending' && (
                    <View style={styles.progressCard}>
                        ...
                        <View style={styles.progressRow}>
                            ...
                            <Text style={styles.progressText}>Documents Under Review</Text>
                        </View>
                        <View style={styles.progressLine} />
                        <View style={styles.progressRow}>
                            ...
                            <Text style={[styles.progressText, { color: '#94A3B8' }]}>Activation</Text>
                        </View>
                    </View>
                )}
                ...
                {status === 'pending' && (
                    <View style={styles.noteCard}>
                        ...
                        <Text style={styles.noteText}>
                            You'll receive an SMS when your application is approved.
                        </Text>
                    </View>
                )}
